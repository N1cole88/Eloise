...
def extract_top_emotions(predictions, top_n=3):
    ...
    for prediction in predictions['predictions']:
        for grouped in prediction['models']['language']['grouped_predictions']:
...
def get_feedback(transcript):
    completion = client.chat.completions.create(
        ...
        messages=[{
            ...
            "role": "user",
            "content": f"Please analyze the following JSON transcript of a conversation, which includes sentiment analysis of both verbal and facial expressions. Provide a summary of the conversation, feedback on what was done well, and feedback on what wasn't done well. Please keep the response under 1500 characters and ignore the twilio rest exception part in the JSON transcript. Here is the JSON transcript: {transcript}"
        }, {
          "role": "assistant",
